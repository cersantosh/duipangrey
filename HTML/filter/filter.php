...
// getting maximum and minimum price

$sql_bikes_price = "select max(new_price) as max_price, min(new_price) as min_price from sell_bikes";
$result_bikes_price = mysqli_query($con, $sql_bikes_price) or die("Query Failed!");

$slq_scooters_price = "select max(new_price) as max_price, min(new_price) as min_price from sell_scooters";
$result_scooters_price = mysqli_query($con, $slq_scooters_price) or die("Query Failed!");

$rows_bikes_price = mysqli_fetch_assoc($result_bikes_price);
$rows_scooters_price = mysqli_fetch_assoc($result_scooters_price);

$bikes_lower_price = $rows_bikes_price['min_price'];
$bikes_upper_price = $rows_bikes_price['max_price'];

$scooters_lower_price = $rows_scooters_price['min_price'];
$scooters_upper_price = $rows_scooters_price['max_price'];

// print($bikes_lower_price);
// print($bikes_upper_price);
// print($scooters_lower_price);
// print($scooters_upper_price);
...
if (isset($_POST['lower_price'])) {
    ...
    // setting lower_price if user only input upper_price
    if ($lower_price == "" && $_POST['upper_price'] != "") {
        $lower_price = ($bikes_lower_price <= $scooters_lower_price) ? $bikes_lower_price : $scooters_lower_price;
    }
...
if (isset($_POST['upper_price'])) {
    ...
    if ($upper_price == "" && $_POST['lower_price'] != "") {
        $upper_price = ($bikes_upper_price >= $scooters_upper_price) ? $bikes_upper_price : $scooters_upper_price;
    }

    // print($_POST['upper_price'] . "<br>");
}

// print($lower_price."<br>");
// print($upper_price."<br>");
...
if (count($array_category) == 1 && $array_category[0] == "bikes") {
    // assigning bikes upper_price if user only give lower_price
    if ($_POST['upper_price'] == "" && $_POST['lower_price'] != "") {
        $upper_price = $bikes_upper_price;
    }
    if ($_POST['upper_price'] != "" && $_POST['lower_price'] == "") {
        $lower_price = $bikes_lower_price;
    }
    $bikes_selected = true;
...
if (count($array_category) == 1 && $array_category[0] == "scooters") {
    $scooters_selected = true;
    // assigning scooters upper_price if user only give lower_price
    if ($_POST['upper_price'] == "" && $_POST['lower_price'] != "") {
        $upper_price = $scooters_upper_price;
    }
    if ($_POST['upper_price'] != "" && $_POST['lower_price'] == "") {
        $lower_price = $scooters_lower_price;
    }
...
// storing filer values provided by uses in session variable
$_SESSION['filter_sql_bikes'] = $sql_bikes;
$_SESSION['filter_sql_scooters'] = $sql_scooters;
