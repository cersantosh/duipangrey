...
unset($_SESSION['signup_access']);
    ...
    $id = $_GET['id'];
    // for getting values from database so that same scooterss won't stored on the database

    $sql_read = "select * from add_to_cart_scooters where email = '$email' and scooters_id = '$id'";
    $result = mysqli_query($con, $sql_read);
    $no_of_rows = mysqli_num_rows($result);
        // for inserting into database if it is not already added to cart
    if($no_of_rows == 0){

        $sql = "insert into add_to_cart_scooters(scooters_id, email) values('$id', '$email')";
